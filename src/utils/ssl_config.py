...
if __name__ == "__main__":
    ...
    # Test with Wazuh default config
    config = SSLConfigurationManager.from_environment("WAZUH_")
    manager = SSLConfigurationManager()
    
    print(f"SSL verification: {config.verify_ssl}")
    print(f"Allow self-signed: {config.allow_self_signed}")
    print(f"Minimum TLS version: {config.min_tls_version}")
    
    is_valid = manager.validate_ssl_config(config)
    print(f"Configuration valid: {is_valid}")
    
    # Auto-detect SSL issues for localhost
    if os.getenv("WAZUH_HOST"):
        host = os.getenv("WAZUH_HOST")
        port = int(os.getenv("WAZUH_PORT", "55000"))
        print(f"\n🔍 Auto-detecting SSL issues for {host}:{port}...")
        results = manager.auto_detect_ssl_issues(host, port)
